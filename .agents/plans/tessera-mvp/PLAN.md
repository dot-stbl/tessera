...
a provider abstraction; frontend trace-detail page shows waterfall + correlated
logs side-by-side like Kibana Observability. MVP-01 deliberately omits Metrics,
Dashboards, and Service Map — those are MVP-02+.
...
**T5.1** `web/apps/console/.env.development` — default API base URL
- `VITE_API_BASE_URL=http://localhost:1990`

**T5.2** `web/apps/console/src/shared/api/` — wire real backend
- Update API client to prefer `VITE_API_BASE_URL` over mock
- Keep mock fallback for tests / offline dev
- File: `web/apps/console/src/shared/api/client.ts`

**T5.3** Trace detail route `/traces/$traceId`
- Fetch trace + correlatedLogs in single call (`GET /api/traces/{traceId}`)
- Render existing `trace-id` + `duration` + `log-level` components

**T5.4** Waterfall component integration
- Use existing `waterfall` component from APM primitives
- Bind to `trace.spans` (reconstructed tree)

**T5.5** Log panel component
- Render `correlatedLogs` alongside waterfall
- Use existing `log-entry` component

**T5.6** Click correlation: log → focus span
- Click on log entry → scroll waterfall to corresponding span + highlight
- File: `web/apps/console/src/features/traces/trace-detail-page.tsx`

**T5.7** Click correlation: span → filter logs
- Click on span in waterfall → filter logs panel to that `spanId` only

**T5.8** Settings page — skeleton
- New route `/settings`

**T5.9** Settings page — `tessera.toml` display (read-only)
- Show `server.port`, `victoria.traces.url`, `victoria.logs.url`, `victoria.tenant`
- Fetched from a dedicated endpoint or static config

**T5.10** Settings page — theme toggle
- Light/dark switcher (already in `lib/preferences`?)

**T5.11** Settings page — language switcher
- EN/RU (already in `lib/i18n`?)
...
- `[.stbl](feat/fe/api): wire real backend as default`
- `[.stbl](feat/fe/traces): trace detail page with log correlation`
- `[.stbl](feat/fe/traces): click correlation log↔span`
- `[.stbl](feat/fe/settings): settings page skeleton`
- `[.stbl](feat/fe/settings): tessera.toml display + theme/language toggle`
...
8. **Frontend UX:** Trace detail page renders waterfall + log panel side-by-side; logs clickable to focus span; spans clickable to filter logs.
...
4. **Frontend mock fallback** — keep mock fallback in dev for offline work, or hard-require real backend?
   - **Default:** keep fallback for offline dev; default `VITE_API_BASE_URL` points to `http://localhost:1990`.

5. **`Tessera.Providers.Victoria` package references** — does it pull Refit directly, or only via `Tessera.Shared.Http` extension?
